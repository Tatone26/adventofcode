...
int main() {
    ...
    while (!feof(f)) {
        ...
        if (not_used_oxygen[i]) nb_to_go_oxygen--;
        if (not_used_co2[i]) nb_to_go_co2--;
        i++;
    }
    number_to_check++;
    while ((nb_to_go_oxygen > 1) || (nb_to_go_co2 >1)) {
        ...
        while (!feof(f)) {
            fgets(buffer, MAX_LEN, f);
            for (int i = 0; i < size_x - 1; i ++){
                if (!not_used_oxygen[j]) compte_oxygen[i][(int) (buffer[i] - '0')]++;
                if (!not_used_co2[j]) compte_co2[i][(int) (buffer[i] - '0')]++;
            }
            j++;
        }
